...
function splitCSS(opt) {
    ...
    opt.html.forEach((html) => {
        ...
        if (html.path) {
            htmlPath =
                path.join(process.cwd(), opt.targetFolder) + "/" + html.path;
        }
        ...
        glob(htmlPath, async (error, files) => {
            ...
            try {
                ...
                const { base: rootFolder } = path.parse(
                    path.resolve(process.cwd(), opt.targetFolder)
                );
                ...
                const fileUpdates = await Promise.all(
                    files.map(async (file) => {
                        ...
                        Array.from(cssLinks)
                            ...
                            .forEach((link) => {
                                const oldHref = url.parse(
                                    link.getAttribute("href")
                                );
                                const { pathname, search } = oldHref;
                                const pathArr = pathname.split("/");
                                const newHref =
                                    (pathArr.splice(-1, 1, cssChunkName),
                                    pathArr).join("/") + search;
                ...
                const concatenateCSS = purgeCSSResult
                    .map((res) => res.css)
                    .join();
                await fs.writeFile(
                    path.join(cssDir, cssChunkName),
